...
//    printf ("Message Serial Number = %" PRIu64 "\n", message->SerialNumber);
	printf("Message Serial Number = %" PRIu64 " | ", message->SerialNumber);
//
...
static void DumpTouchActiveArea (Message * message)
{
    ...
    if (message->Error)
        ...
        if (touchActiveAreaMessage->HasX) {
            printf ("   X lower boundary = %u",  touchActiveAreaMessage->LowerBoundaryX);
            printf ("   X upper boundary = %u\n",touchActiveAreaMessage->UpperBoundaryX);
        }
        if (touchActiveAreaMessage->HasY) {
            printf ("   Y lower boundary = %u",  touchActiveAreaMessage->LowerBoundaryY);
...
static void DumpTouchMessage (Message * message)
{
    ...
    printf ("Id: %2d, Event: %7s", touchMessage->Id, eventString);

    if (touchMessage->HasX || touchMessage->HasY || touchMessage->HasZ) {
        printf (", ");
        ...
        if (touchMessage->HasZ && (touchMessage->HasX || touchMessage->HasY))
            printf (", ");
        if (touchMessage->HasZ)
            printf ("Z: %8d", touchMessage->Z);
    }

    if (touchMessage->HasSizeX || touchMessage->HasSizeY || touchMessage->HasSizeZ) {
        printf (", ");
        ...
        if (touchMessage->HasSizeZ && (touchMessage->HasSizeX || touchMessage->HasSizeY))
            printf (", ");
...
static void DumpNumberOfTrackedObjectsMessage (Message * message)
{
    NumberOfTrackedObjectsMessage * numberOfTrackedObjectsMessage = (NumberOfTrackedObjectsMessage *)message;
...
static void DumpFingerFrequencyMessage (Message * message)
{
    FingerFrequencyMessage * fingerFrequencyMessage = (FingerFrequencyMessage *)message;
...
static void DumpIdleFrequencyMessage (Message * message)
{
    IdleFrequencyMessage * idleFrequencyMessage = (IdleFrequencyMessage *)message;
...
static void DumpDetectedObjectSizeRestrictionMessage (Message * message)
{
    DetectedObjectSizeRestrictionMessage * sizeMessage = (DetectedObjectSizeRestrictionMessage *) message;
...
static void DumpMessageError (Message * message)
{
    printf ("**  Error: (%d) %s\n", message->ErrorCode, ErrorString (message->ErrorCode));
}
